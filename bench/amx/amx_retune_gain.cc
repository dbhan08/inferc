...
int main(){
  ...
  const Shape shapes[]={
    ...
    {"GPT-FFN2",     2048, 8192},
    {"Tiny-FFN2",    2048, 5632},
    {"Llama-FFN2",   4096, 11008},
  };
  const int NCS[]={512,64};
  const int KCS[]={1024,2048};
  std::printf("# M=%d, best of Kc in {1024,2048}, median-of-21, Nc in {512,64}\n",M);
  for(const Shape&s:shapes){
    ...
    for(int Nc:NCS){
      ...
      for(int Kc:KCS){
        ...
        std::printf("GAIN,%s,%lld,%lld,%d,%d,%.0f,%.1e\n",s.name,(long long)N,(long long)K,Nc,Kc,g,md);
      }
    }
  }
}
